"use client";
import { IInstitutionDetailsCardProps } from "@/types";
...
const InstituteDetailsCard = ({
  ...
  description,
  instituteUrl,
}: IInstitutionDetailsCardProps) => {
  return (
    <div className="md:w-[16.875rem] tablet:w-[24.375rem]  flex-shrink-0 rounded-[1.25rem] bg-card-background">
      {/* image */}
      ...
      {/* href={`/${instituteUrl}` */}
      {/* check if really need this type of url, if its required, need to create another api that query data through username */}
      <Link href={`/${id}`}>
        <Button
          btnColor="blue"
